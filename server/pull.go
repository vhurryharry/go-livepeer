...
func (s *LivepeerServer) Pull(fname string, streamName core.ManifestID) {
	fmt.Println("pulling ", fname, " to ", BroadcastJobVideoProfiles)
...
func runSegmenter(node *core.LivepeerNode, cxn *rtmpConnection, fname string) {
	...
	rd, wr, err := os.Pipe()
	if err != nil {
		fmt.Println("error creating pipe err=", err)
		return
	}
	closeFds := func() {
		rd.Close()
	...
	go func() {
		...
		for {
			...
			case <-feederTermCh:
				...
				if head != nil || !ioTerminated {
					fmt.Println("unexpected exit for feeder; may dangle")
				}
				return
			}
		}
		...
		go func(worker int) {
			...
			for {
				...
				case seg := <-workerCh:
					...
					readyCh <- workerCh
				case <-ctx.Done():
					fmt.Println("terminating worker ", worker)
					workerTermCh <- struct{}{}
					return
				}
			}
		...
		[]ffmpeg.TranscodeOptions{{
			...
			Muxer: ffmpeg.ComponentOptions{
				...
				Opts: map[string]string{
					...
					"segment_time":      "2",
					// TODO fix need for trailing slash
					"segment_list_entry_prefix": filepath.Join(base...) + "/",
				}}}})
	if err != nil {
		fmt.Println("error segmenting err=", err)
	}
	closeFds()
	for i := 0; i < nbWorkers; i++ {
		<-workerTermCh
